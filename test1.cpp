...
// Pyramid level auto = 5
int LK_wd_size =4;					// Lucas-Kanade window size
int level_guessing =30;				// Number of iteration
float thresh =10000000000;				// Harris threshold
...
unsigned char *buffer;// = (unsigned char*)mmap(NULL, buf_v4l2.length, PROT_READ | PROT_WRITE, MAP_SHARED, fd_cam_main, buf_v4l2.m.offset);
int frame_count = 0;
int network_socket;
...
//        clock_gettime(CLOCK_MONOTONIC, &time3);			// Get time 2

//         float total_time_dif = BILLION * (time3.tv_sec - time2.tv_sec) + time3.tv_nsec - time2.tv_nsec;
//         total_time_dif /= BILLION;
    frame_count++;
...
int send_to_server(){
	...
	//Open file that we wish to transfer
	FILE *fp = fopen(IMG_TEMP,"rb");
	if (fp == NULL)
	{
	...
	while (1)
	{
		...
		//printf("Bytes read %d \n", nread);	
		// If read sucess, send data
		if (nread > 0)
		{
			//printf("Sending \n");
			write(network_socket, buff, nread);
		}
		if (nread < 256)
		{
			if (feof(fp))
...
//	int network_socket;
    ...
    int32_t *Sxy_p = Sxy;
    int lf = 200;
    Mat binary(480,640,CV_8U);  //create blank matrix
...
//    unsigned char *
   ...
   // printf("\n=== buffer.length: %d ===; buf_v4l2.length: %d == \n",strlen(buffer),buf_v4l2.length);
...
/*** kien fix 18/09/2018 *** calculate density and send final result to server ******/	
	
	av_density_arr[lan_do] = av_density;
	++lan_do;

	if (lan_do < SO_LAN_DO ) continue; // skip all below command 
